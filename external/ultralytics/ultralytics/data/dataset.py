...
class YOLODataset(BaseDataset):
    ...
    Attributes:
        ...
        use_polygon (bool): Indicates if quadrilateral polygon vertices should be used.
    ...
    def __init__(self, *args, data: dict | None = None, task: str = "detect", **kwargs):
        ...
        Args:
            ...
            task (str): Task type, one of 'detect', 'segment', 'pose', 'obb', or 'polygon'.
        ...
        self.data = data
        assert sum((self.use_segments, self.use_keypoints, self.use_obb, self.use_polygon)) <= 1, (
            "Can not use more than one annotation task."
        )
    ...
    def update_labels_info(self, label: dict) -> dict:
        ...
        if self.use_polygon:
            if len(segments) > 0:
                if any(np.asarray(s).shape != (4, 2) for s in segments):
                    raise ValueError("Polygon task requires labels with exactly 4 xy vertices per object.")
                segments = np.stack(segments, axis=0).astype(np.float32, copy=False)
            elif len(bboxes):
                raise ValueError("Polygon task requires labels in 'class x1 y1 x2 y2 x3 y3 x4 y4' format.")
            else:
