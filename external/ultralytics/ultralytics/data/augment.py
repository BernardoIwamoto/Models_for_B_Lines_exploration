...
class Format(BaseTransform):
    ...
    Attributes:
        ...
        return_polygon (bool): Whether to return ordered quadrilateral vertices.
    ...
    def __init__(
        ...
        Args:
            ...
            return_polygon (bool): If True, returns quadrilateral vertices with shape (N, 8).
            mask_ratio (int): Downsample ratio for masks.
    ...
    def apply_instances(self, labels: dict[str, Any], params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if self.return_polygon:
            labels["polygons"] = torch.from_numpy(instances.segments.reshape(nl, 8)) if nl else torch.zeros((nl, 8))
            if self.normalize and nl:
                labels["polygons"][:, 0::2] /= w
                labels["polygons"][:, 1::2] /= h
